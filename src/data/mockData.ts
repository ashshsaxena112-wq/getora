import { Category, Store, Product, Coupon, Address, Order } from '../types';

export const INITIAL_CATEGORIES: Category[] = [
  {
    id: 'cat-grocery',
    name: 'Grocery & Essentials',
    slug: 'grocery-essentials',
    description: 'Fresh daily, delivered instantly.',
    imageUrl: 'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=800&auto=format&fit=crop&q=80',
    iconName: 'Apple',
    storeCount: 14
  },
  {
    id: 'cat-electronics',
    name: 'Electronics',
    slug: 'electronics',
    description: 'Headphones, cables, chargers & audio gear.',
    imageUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800&auto=format&fit=crop&q=80',
    iconName: 'Headphones',
    storeCount: 18
  },
  {
    id: 'cat-electrical',
    name: 'Electrical',
    slug: 'electrical',
    description: 'Bulbs, wires, switches & lighting.',
    imageUrl: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format&fit=crop&q=80',
    iconName: 'Zap',
    storeCount: 9
  },
  {
    id: 'cat-hardware',
    name: 'Hardware',
    slug: 'hardware',
    description: 'Tools, plumbing, fixtures & fasteners.',
    imageUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80',
    iconName: 'Wrench',
    storeCount: 11
  },
  {
    id: 'cat-mobile',
    name: 'Mobile Accessories',
    slug: 'mobile-accessories',
    description: 'Cases, screen guards, chargers & stands.',
    imageUrl: 'https://images.unsplash.com/photo-1580910051074-3eb694886505?w=800&auto=format&fit=crop&q=80',
    iconName: 'Smartphone',
    storeCount: 22
  },
  {
    id: 'cat-stationery',
    name: 'Stationery',
    slug: 'stationery',
    description: 'Notebooks, pens, office & art supplies.',
    imageUrl: 'https://images.unsplash.com/photo-1583485088034-697b5bc54ccd?w=800&auto=format&fit=crop&q=80',
    iconName: 'BookOpen',
    storeCount: 8
  },
  {
    id: 'cat-home',
    name: 'Home Essentials',
    slug: 'home-essentials',
    description: 'Kitchen, storage & cleaning utilities.',
    imageUrl: 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=800&auto=format&fit=crop&q=80',
    iconName: 'Home',
    storeCount: 15
  },
  {
    id: 'cat-auto',
    name: 'Auto Accessories',
    slug: 'auto-accessories',
    description: 'Helmets, bike polish, car care & bulbs.',
    imageUrl: 'https://images.unsplash.com/photo-1486006920555-c77dce18193b?w=800&auto=format&fit=crop&q=80',
    iconName: 'Car',
    storeCount: 7
  },
  {
    id: 'cat-pet',
    name: 'Pet Supplies',
    slug: 'pet-supplies',
    description: 'Pet food, treats, toys & grooming.',
    imageUrl: 'https://images.unsplash.com/photo-1583337130417-3346a1be7dee?w=800&auto=format&fit=crop&q=80',
    iconName: 'Heart',
    storeCount: 6
  },
  {
    id: 'cat-care',
    name: 'Personal Care',
    slug: 'personal-care',
    description: 'Grooming, skincare & pharmacy basics.',
    imageUrl: 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=800&auto=format&fit=crop&q=80',
    iconName: 'Sparkles',
    storeCount: 16
  }
];

export const INITIAL_STORES: Store[] = [
  {
    id: 'store-voltix',
    name: 'Voltix Electricals',
    slug: 'voltix-electricals',
    tagline: 'Premium Industrial & Home Lighting',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    logoUrl: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=200&auto=format&fit=crop&q=80',
    bannerUrl: 'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?w=1200&auto=format&fit=crop&q=80',
    rating: 4.8,
    reviewCount: 124,
    openTime: '09:00 AM',
    closeTime: '10:00 PM',
    isOpen: true,
    address: '14, 80 Feet Road, 4th Block',
    locality: 'Koramangala, Bengaluru',
    distanceKm: 2.4,
    deliveryEtaMin: 20,
    deliveryFee: 40,
    freeDeliveryThreshold: 499,
    offerText: '₹50 OFF on ₹399',
    featured: true
  },
  {
    id: 'store-abc-electrical',
    name: 'ABC Electrical Store',
    slug: 'abc-electrical-store',
    tagline: 'Neighborhood Electric & Hardware Experts',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    logoUrl: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=200&auto=format&fit=crop&q=80',
    bannerUrl: 'https://images.unsplash.com/photo-1555421689-491a97ff2040?w=1200&auto=format&fit=crop&q=80',
    rating: 4.7,
    reviewCount: 98,
    openTime: '08:30 AM',
    closeTime: '09:30 PM',
    isOpen: true,
    address: '52, 1st Cross, Indiranagar',
    locality: 'Indiranagar, Bengaluru',
    distanceKm: 1.2,
    deliveryEtaMin: 20,
    deliveryFee: 40,
    freeDeliveryThreshold: 399,
    offerText: '15% OFF on Fans',
    featured: true
  },
  {
    id: 'store-natures-fresh',
    name: "Nature's Fresh Grocery",
    slug: 'natures-fresh-grocery',
    tagline: 'Farm Fresh Veggies, Fruits & Daily Dairy',
    categoryId: 'cat-grocery',
    categoryName: 'Grocery & Essentials',
    logoUrl: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=200&auto=format&fit=crop&q=80',
    bannerUrl: 'https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=1200&auto=format&fit=crop&q=80',
    rating: 4.9,
    reviewCount: 312,
    openTime: '07:00 AM',
    closeTime: '11:00 PM',
    isOpen: true,
    address: '88, 12th Main, HAL 2nd Stage',
    locality: 'Indiranagar, Bengaluru',
    distanceKm: 0.5,
    deliveryEtaMin: 12,
    deliveryFee: 0,
    freeDeliveryThreshold: 199,
    offerText: '10% OFF',
    featured: true
  },
  {
    id: 'store-techhub',
    name: 'TechHub Electronics',
    slug: 'techhub-electronics',
    tagline: 'Authorized Accessories, Audio & Cables',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    logoUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&auto=format&fit=crop&q=80',
    bannerUrl: 'https://images.unsplash.com/photo-1526738549149-8e07eca6c147?w=1200&auto=format&fit=crop&q=80',
    rating: 4.6,
    reviewCount: 85,
    openTime: '10:00 AM',
    closeTime: '09:00 PM',
    isOpen: true,
    address: '22, Brigade Road',
    locality: 'MG Road, Bengaluru',
    distanceKm: 2.8,
    deliveryEtaMin: 30,
    deliveryFee: 60,
    freeDeliveryThreshold: 999,
    offerText: 'Special Weekend Deals',
    featured: true
  },
  {
    id: 'store-sonix',
    name: 'Sonix Official Store',
    slug: 'sonix-official-store',
    tagline: 'Flagship Acoustics, ANC Headphones & Earbuds',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    logoUrl: 'https://images.unsplash.com/photo-1546435770-a3e426bf472b?w=200&auto=format&fit=crop&q=80',
    bannerUrl: 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=1200&auto=format&fit=crop&q=80',
    rating: 4.9,
    reviewCount: 1284,
    openTime: '10:00 AM',
    closeTime: '10:00 PM',
    isOpen: true,
    address: '101, 100 Feet Road',
    locality: 'Indiranagar, Bengaluru',
    distanceKm: 1.5,
    deliveryEtaMin: 15,
    deliveryFee: 0,
    freeDeliveryThreshold: 500,
    offerText: 'Flat 32% OFF',
    featured: true
  },
  {
    id: 'store-ganesh-hardware',
    name: 'Shree Ganesh Hardware & Tools',
    slug: 'shree-ganesh-hardware',
    tagline: 'Plumbing, Power Tools & Fasteners',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    logoUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=200&auto=format&fit=crop&q=80',
    bannerUrl: 'https://images.unsplash.com/photo-1504148455328-c376907d081c?w=1200&auto=format&fit=crop&q=80',
    rating: 4.8,
    reviewCount: 76,
    openTime: '08:00 AM',
    closeTime: '09:00 PM',
    isOpen: true,
    address: '33, CMH Road',
    locality: 'Indiranagar, Bengaluru',
    distanceKm: 1.8,
    deliveryEtaMin: 25,
    deliveryFee: 35,
    freeDeliveryThreshold: 499,
    offerText: '₹100 OFF on Tools',
    featured: false
  }
];

export const INITIAL_PRODUCTS: Product[] = [
  // Sonix Flagship (Screenshot 3)
  {
    id: 'prod-sonix-nc700',
    retailerId: 'store-sonix',
    storeId: 'store-sonix',
    storeName: 'Sonix Official Store',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    subCategory: 'Headphones & ANC',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&auto=format&fit=crop&q=80',
    name: 'Sonix Pro NC-700 Active Noise Cancelling Headphones',
    variantLabel: 'Matte Black',
    unitType: 'count',
    unitOptions: [1, 2, 3, 5],
    slug: 'sonix-pro-nc-700-active-noise-cancelling-headphones',
    brand: 'SONIX',
    description: 'Engineered for exceptional studio sound and ultra-quiet isolation. Equipped with custom tuned 40mm beryllium drivers, smart active noise cancellation, low-latency gaming mode, and featherlight plush ear cushions for all-day comfort.',
    price: 12999,
    sellingPrice: 12999,
    mrp: 18999,
    discountPercent: 32,
    inStock: true,
    stockQuantity: 18,
    unit: '1 unit',
    packInfo: 'Includes travel case, AUX cable & USB-C fast charging cord',
    images: [
      'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1484704849700-f032a568e944?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1583394838336-acd977736f90?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1546435770-a3e426bf472b?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Battery', value: '40 Hours Playtime' },
      { label: 'Noise Cancelling', value: 'Active Hybrid ANC' },
      { label: 'Connectivity', value: 'Bluetooth 5.3 + Multipoint' },
      { label: 'Microphone', value: '6-Mic Beamforming System' }
    ],
    colors: ['#1A1A1A', '#F0F0F0', '#1B2A4A'],
    isPopular: true,
    rating: 4.8,
    reviewCount: 1284,
    deliveryEtaMin: 15
  },
  {
    id: 'prod-sonix-earbuds',
    retailerId: 'store-sonix',
    storeId: 'store-sonix',
    storeName: 'Sonix Official Store',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    subCategory: 'TWS Earbuds',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=200&auto=format&fit=crop&q=80',
    name: 'Sonix Pulse True Wireless Earbuds with ANC',
    variantLabel: 'Obsidian Black',
    unitType: 'count',
    unitOptions: [1, 2, 3, 4],
    slug: 'sonix-pulse-tws-earbuds',
    brand: 'SONIX',
    description: 'Ultra-low latency wireless earbuds with environmental noise cancellation for crystal clear calls and deep punchy bass.',
    price: 3499,
    sellingPrice: 2999,
    mrp: 4999,
    discountPercent: 40,
    inStock: true,
    stockQuantity: 25,
    unit: '1 pair',
    packInfo: 'Charging Case + 3 Size Ear Tips',
    images: [
      'https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Battery', value: '32h Total with Case' },
      { label: 'Waterproof', value: 'IPX5 Sweat Resistant' }
    ],
    isPopular: true,
    rating: 4.7,
    reviewCount: 642,
    deliveryEtaMin: 15
  },

  // Voltix Electricals Products (Screenshot 4)
  {
    id: 'prod-philips-9w-bulb',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'LED Bulbs & Lighting',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1550985543-f47f38aeee65?w=200&auto=format&fit=crop&q=80',
    name: 'Philips 9W B22 LED Bulb, Cool Day Light',
    variantLabel: '9W Cool White',
    unitType: 'count',
    unitOptions: [1, 2, 3, 5, 10],
    slug: 'philips-9w-b22-led-bulb',
    brand: 'Philips',
    description: 'Bright 6500K cool day light with EyeComfort technology. 85% energy savings over standard incandescent bulbs, lasts up to 15,000 hours with built-in surge protection.',
    price: 149,
    sellingPrice: 149,
    mrp: 199,
    discountPercent: 25,
    inStock: true,
    stockQuantity: 120,
    unit: 'Pack of 1',
    packInfo: 'Base: B22 Pin Cap | Wattage: 9W',
    images: [
      'https://images.unsplash.com/photo-1550985543-f47f38aeee65?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1532009324734-20a7a5813719?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Wattage', value: '9W (825 Lumens)' },
      { label: 'Color Temp', value: '6500K Cool White' },
      { label: 'Warranty', value: '1 Year Manufacturer' },
      { label: 'Holder Type', value: 'B22 Indian Pin Base' }
    ],
    isPopular: true,
    rating: 4.7,
    reviewCount: 340,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-philips-12w-bulb',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'LED Bulbs & Lighting',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1550985543-f47f38aeee65?w=200&auto=format&fit=crop&q=80',
    name: 'Philips 12W Inverter Rechargeable Emergency LED Bulb',
    variantLabel: '12W Emergency Backup',
    unitType: 'count',
    unitOptions: [1, 2, 4],
    slug: 'philips-12w-inverter-bulb',
    brand: 'Philips',
    description: 'Emergency LED bulb with 2200mAh lithium-ion battery. Automatically turns on during power cuts with up to 4 hours of continuous backup lighting.',
    price: 499,
    sellingPrice: 399,
    mrp: 650,
    discountPercent: 38,
    inStock: true,
    stockQuantity: 40,
    unit: '1 pc',
    packInfo: 'B22 Cap | 4hr Backup',
    images: [
      'https://images.unsplash.com/photo-1550985543-f47f38aeee65?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Battery Backup', value: 'Up to 4 Hours' },
      { label: 'Charging Time', value: '8-10 Hours Auto-charge' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 112,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-havells-wire',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Wires & Cables',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=200&auto=format&fit=crop&q=80',
    name: 'Havells 1.5 sq mm Single Core FR PVC Wire (90m)',
    variantLabel: '1.5 sq mm (Red)',
    unitType: 'count',
    unitOptions: [1, 2, 3, 5],
    slug: 'havells-1-5-sq-mm-wire-red',
    brand: 'Havells',
    description: 'High conductivity pure electrolytic copper conductors insulated with heat-resistant Flame Retardant (FR) grade PVC compound for absolute residential & commercial electrical safety.',
    price: 1250,
    sellingPrice: 1250,
    mrp: 1490,
    discountPercent: 16,
    inStock: true,
    stockQuantity: 45,
    unit: '90m Length Coil',
    packInfo: 'Color: Red | 1100V Grade ISI Certified',
    images: [
      'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Conductor', value: '100% Pure Copper' },
      { label: 'Insulation', value: 'Flame Retardant FR PVC' },
      { label: 'Length', value: '90 Meters' },
      { label: 'Certification', value: 'ISI Mark IS:694' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 89,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-havells-wire-25',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Wires & Cables',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=200&auto=format&fit=crop&q=80',
    name: 'Havells 2.5 sq mm Single Core FR Wire (90m)',
    variantLabel: '2.5 sq mm (Blue)',
    unitType: 'count',
    unitOptions: [1, 2, 4],
    slug: 'havells-2-5-sq-mm-wire-blue',
    brand: 'Havells',
    description: 'Heavy gauge 2.5 sq mm electrolytic copper wire engineered for air conditioners, water heaters and power sockets.',
    price: 1980,
    sellingPrice: 1850,
    mrp: 2300,
    discountPercent: 19,
    inStock: true,
    stockQuantity: 30,
    unit: '90m Coil',
    packInfo: 'Color: Blue | Heavy Load Rating',
    images: [
      'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Gauge', value: '2.5 sq mm' },
      { label: 'Ideal for', value: 'AC / Geyser / Power Sockets' }
    ],
    isPopular: false,
    rating: 4.9,
    reviewCount: 42,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-atomberg-fan',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Fans & Ventilation',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1618220179428-22790b461013?w=200&auto=format&fit=crop&q=80',
    name: 'Atomberg Renesa 1200mm BLDC Motor Ceiling Fan',
    variantLabel: '1200mm Matte Black',
    unitType: 'count',
    unitOptions: [1, 2, 3],
    slug: 'atomberg-renesa-1200mm-bldc-fan',
    brand: 'Atomberg',
    description: 'Super energy efficient BLDC motor consuming only 28W at top speed. Saves up to ₹1,500/year on electricity. Comes with smart remote control with Sleep, Boost and Timer modes.',
    price: 3490,
    sellingPrice: 3490,
    mrp: 4750,
    discountPercent: 26,
    inStock: true,
    stockQuantity: 22,
    unit: '1 unit',
    packInfo: 'Finish: Matte Black | Span: 1200mm (48 Inch)',
    images: [
      'https://images.unsplash.com/photo-1618220179428-22790b461013?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Motor', value: 'Energy Saver BLDC (28W)' },
      { label: 'Air Delivery', value: '235 CMM @ 360 RPM' },
      { label: 'Control', value: 'Smart Remote Included' },
      { label: 'Warranty', value: '2+1 Year On-site' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 420,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-legrand-switch-plate',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Switches & Plates',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=200&auto=format&fit=crop&q=80',
    name: 'Legrand Myrius Modular Switch Plate (3 Module)',
    variantLabel: '3 Module Gunmetal',
    unitType: 'count',
    unitOptions: [1, 2, 5, 10],
    slug: 'legrand-myrius-modular-switch-plate',
    brand: 'Legrand',
    description: 'Sleek gunmetal grey textured finish with fire-retardant virgin polycarbonate material. Scratch-resistant surface designed for modern minimalist interiors.',
    price: 420,
    sellingPrice: 420,
    mrp: 510,
    discountPercent: 18,
    inStock: true,
    stockQuantity: 80,
    unit: '1 unit',
    packInfo: 'Finish: Gunmetal Grey | 3 Modules Grid',
    images: [
      'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Material', value: 'Fire-Safe Polycarbonate' },
      { label: 'Finish', value: 'Matte Gunmetal' },
      { label: 'Capacity', value: '3 Modules' }
    ],
    isPopular: false,
    rating: 4.6,
    reviewCount: 45,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-anchor-switch-1way',
    retailerId: 'store-voltix',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeLocality: 'Koramangala',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Switches & Plates',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=200&auto=format&fit=crop&q=80',
    name: 'Anchor Roma Classic 6A 1-Way Modular Switch',
    variantLabel: '6A Pure White',
    unitType: 'count',
    unitOptions: [1, 2, 5, 10, 20],
    slug: 'anchor-roma-6a-switch',
    brand: 'Anchor by Panasonic',
    description: 'Spark-shielded smooth push modular switch with silver cadmium oxide contacts rated for 100,000 continuous operations.',
    price: 48,
    sellingPrice: 38,
    mrp: 55,
    discountPercent: 30,
    inStock: true,
    stockQuantity: 150,
    unit: '1 pc',
    packInfo: 'Rating: 6A 240V | Flame Retardant Body',
    images: [
      'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Rating', value: '6A 240V AC' },
      { label: 'Lifespan', value: '100,000 Cycles' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 190,
    deliveryEtaMin: 20
  },

  // Shree Ganesh Hardware & Tools (Hardware Store)
  {
    id: 'prod-nails-9mm',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Nails & Fasteners',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=200&auto=format&fit=crop&q=80',
    name: 'Hardened Concrete Steel Nails (9mm)',
    variantLabel: 'Nails 9mm',
    unitType: 'weight',
    unitOptions: [50, 100, 250, 500, 1000],
    weightUnit: 'g',
    slug: 'hardened-steel-nails-9mm',
    brand: 'Tata Agrico',
    description: 'High-tensile zinc-galvanized carbon steel concrete nails (9mm length). Corrosion resistant with diamond point penetration for masonry, brick and timber.',
    price: 120,
    sellingPrice: 95,
    mrp: 140,
    discountPercent: 32,
    inStock: true,
    stockQuantity: 250,
    unit: 'Sold by Weight (100g base)',
    packInfo: 'Gauge: 9mm | Zinc-Coated Anti-Rust',
    images: [
      'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Length', value: '9mm' },
      { label: 'Material', value: 'Grade 1060 Carbon Steel' },
      { label: 'Coating', value: 'Zinc Electroplated' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 135,
    deliveryEtaMin: 25
  },
  {
    id: 'prod-nails-10mm',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Nails & Fasteners',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=200&auto=format&fit=crop&q=80',
    name: 'Heavy Duty Masonry Steel Nails (10mm)',
    variantLabel: 'Nails 10mm',
    unitType: 'weight',
    unitOptions: [50, 100, 250, 500, 1000],
    weightUnit: 'g',
    slug: 'heavy-duty-masonry-nails-10mm',
    brand: 'Tata Agrico',
    description: 'Extra sturdy 10mm fluted shank concrete nails with countersunk head for high-load anchoring in solid stone and reinforced concrete.',
    price: 135,
    sellingPrice: 105,
    mrp: 160,
    discountPercent: 34,
    inStock: true,
    stockQuantity: 200,
    unit: 'Sold by Weight (100g base)',
    packInfo: 'Gauge: 10mm | Fluted Shank Anti-Slip',
    images: [
      'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Length', value: '10mm' },
      { label: 'Shank', value: 'Fluted Spiral Shank' },
      { label: 'Hardness', value: '52-54 HRC' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 92,
    deliveryEtaMin: 25
  },
  {
    id: 'prod-drywall-screws',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Nails & Fasteners',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=200&auto=format&fit=crop&q=80',
    name: 'Black Phosphate Bugle Head Drywall Screws (25mm)',
    variantLabel: 'Screws 25mm (Black)',
    unitType: 'count',
    unitOptions: [10, 25, 50, 100, 200],
    slug: 'drywall-screws-25mm-black',
    brand: 'Hilti Pro',
    description: 'Self-tapping Philips drive drywall screws with coarse threads for plasterboard to wood and light gauge steel framing.',
    price: 80,
    sellingPrice: 65,
    mrp: 95,
    discountPercent: 31,
    inStock: true,
    stockQuantity: 500,
    unit: 'Pack of 25 pcs',
    packInfo: '25mm Length | #6 Diameter',
    images: [
      'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Drive', value: 'Philips Cross Recess #2' },
      { label: 'Finish', value: 'Black Phosphate' }
    ],
    isPopular: false,
    rating: 4.7,
    reviewCount: 54,
    deliveryEtaMin: 25
  },
  {
    id: 'prod-godrej-door-lock',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Door Locks & Security',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=200&auto=format&fit=crop&q=80',
    name: 'Godrej Classic Cylindrical Door Lock (Stainless Steel)',
    variantLabel: 'SS Satin Finish 60mm',
    unitType: 'count',
    unitOptions: [1, 2, 3, 5],
    slug: 'godrej-cylindrical-door-lock-ss',
    brand: 'Godrej',
    description: 'Premium cylindrical lock with automatic deadlocking brass latchbolt and 3 dimple computer keys for main entrance and bedroom doors.',
    price: 1250,
    sellingPrice: 1049,
    mrp: 1450,
    discountPercent: 28,
    inStock: true,
    stockQuantity: 35,
    unit: '1 Lock + 3 Keys',
    packInfo: 'Backset: 60mm | 3 Brass Keys Included',
    images: [
      'https://images.unsplash.com/photo-1558002038-1055907df827?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Keys', value: '3 Dimple High-Security Keys' },
      { label: 'Door Thickness', value: '32mm to 45mm' },
      { label: 'Warranty', value: '2 Years Manufacturer' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 164,
    deliveryEtaMin: 25
  },
  {
    id: 'prod-godrej-navtal-brass',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Door Locks & Security',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=200&auto=format&fit=crop&q=80',
    name: 'Godrej Nav-Tal 7-Lever Solid Brass Padlock (60mm)',
    variantLabel: 'Brass 60mm (7-Lever)',
    unitType: 'count',
    unitOptions: [1, 2, 4],
    slug: 'godrej-navtal-7lever-padlock',
    brand: 'Godrej',
    description: 'Iconic 7-lever solid brass padlock with hardened steel shackle that resists hacksaw cutting and lever picking.',
    price: 650,
    sellingPrice: 549,
    mrp: 750,
    discountPercent: 26,
    inStock: true,
    stockQuantity: 40,
    unit: '1 Padlock + 3 Keys',
    packInfo: 'Solid Brass 60mm Body',
    images: [
      'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Body', value: 'Solid Extruded Brass' },
      { label: 'Levers', value: '7 Brass Levers' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 215,
    deliveryEtaMin: 25
  },
  {
    id: 'prod-bosch-drill',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Hand & Power Tools',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1504148455328-c376907d081c?w=200&auto=format&fit=crop&q=80',
    name: 'Bosch GSB 500W Professional Impact Drill Kit',
    variantLabel: '500W + 100pc Kit',
    unitType: 'count',
    unitOptions: [1, 2],
    slug: 'bosch-gsb-500w-impact-drill-kit',
    brand: 'Bosch',
    description: 'Powerful 500W forward/reverse impact drill machine with complete 100-piece accessory kit for masonry, wood and metal drilling.',
    price: 3699,
    sellingPrice: 3699,
    mrp: 4500,
    discountPercent: 18,
    inStock: true,
    stockQuantity: 15,
    unit: '1 Kit',
    packInfo: 'Includes sturdy carry case & 100 attachments',
    images: [
      'https://images.unsplash.com/photo-1504148455328-c376907d081c?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Power', value: '500 Watts Motor' },
      { label: 'Chuck Size', value: '10 mm Keyed' },
      { label: 'Warranty', value: '6 Months Bosch Warranty' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 77,
    deliveryEtaMin: 25
  },
  {
    id: 'prod-fevicol-sh',
    retailerId: 'store-ganesh-hardware',
    storeId: 'store-ganesh-hardware',
    storeName: 'Shree Ganesh Hardware & Tools',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-hardware',
    categoryName: 'Hardware',
    subCategory: 'Adhesives & Sealants',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=200&auto=format&fit=crop&q=80',
    name: 'Fevicol SH Ultimate Synthetic Resin Wood Adhesive',
    variantLabel: '1 kg Jar',
    unitType: 'weight',
    unitOptions: [250, 500, 1000, 2000],
    weightUnit: 'g',
    slug: 'fevicol-sh-wood-adhesive-1kg',
    brand: 'Pidilite',
    description: 'India’s most trusted wood adhesive. Unbeatable bonding strength for wood, plywood, laminate, MDF and veneers.',
    price: 310,
    sellingPrice: 260,
    mrp: 340,
    discountPercent: 23,
    inStock: true,
    stockQuantity: 60,
    unit: 'Sold by Weight (1kg base)',
    packInfo: 'Grade: Synthetic Resin Adhesive',
    images: [
      'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Setting Time', value: '4-6 Hours' },
      { label: 'Bond Strength', value: 'Superior Tensile Hold' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 140,
    deliveryEtaMin: 25
  },

  // Grocery Products (Screenshot 1 & 5)
  {
    id: 'prod-organic-broccoli',
    retailerId: 'store-natures-fresh',
    storeId: 'store-natures-fresh',
    storeName: "Nature's Fresh Grocery",
    storeLocality: 'Indiranagar',
    categoryId: 'cat-grocery',
    categoryName: 'Grocery & Essentials',
    subCategory: 'Fresh Vegetables',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=200&auto=format&fit=crop&q=80',
    name: 'Fresh Organic Green Broccoli',
    variantLabel: '500g Florets',
    unitType: 'weight',
    unitOptions: [250, 500, 1000],
    weightUnit: 'g',
    slug: 'fresh-organic-green-broccoli',
    brand: 'Farm Fresh',
    description: 'Crisp, pesticide-free farm-fresh broccoli florets packed with antioxidants, Vitamin C and dietary fiber. Harvested at sunrise.',
    price: 89,
    sellingPrice: 89,
    mrp: 120,
    discountPercent: 26,
    inStock: true,
    stockQuantity: 40,
    unit: '500g Pack',
    packInfo: 'Hydroponically grown & sanitized',
    images: [
      'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=800&auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1584270354949-c26b0d5b4a0c?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Origin', value: 'Ooty Farm Direct' },
      { label: 'Shelf Life', value: '4 Days refrigerated' },
      { label: 'Weight', value: '500g Approx' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 156,
    deliveryEtaMin: 12
  },
  {
    id: 'prod-alphonso-mangoes',
    retailerId: 'store-natures-fresh',
    storeId: 'store-natures-fresh',
    storeName: "Nature's Fresh Grocery",
    storeLocality: 'Indiranagar',
    categoryId: 'cat-grocery',
    categoryName: 'Grocery & Essentials',
    subCategory: 'Fruits & Organics',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1553279768-865429fa0078?w=200&auto=format&fit=crop&q=80',
    name: 'Ratnagiri Alphonso Mangoes (GI Tagged)',
    variantLabel: 'Box of 6 Pcs',
    unitType: 'count',
    unitOptions: [1, 2, 3, 5],
    slug: 'ratnagiri-alphonso-mangoes',
    brand: 'Nature Choice',
    description: 'Naturally ripened, sweet and aromatic original Ratnagiri Alphonso mangoes. Chemical-free and carbide-free ripening.',
    price: 699,
    sellingPrice: 699,
    mrp: 850,
    discountPercent: 18,
    inStock: true,
    stockQuantity: 25,
    unit: 'Box of 6 Pcs',
    packInfo: 'A-Grade Jumbo Size (250g+ each)',
    images: [
      'https://images.unsplash.com/photo-1553279768-865429fa0078?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Ripening', value: '100% Grass Ripened' },
      { label: 'Origin', value: 'Ratnagiri, Maharashtra' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 388,
    deliveryEtaMin: 12
  },
  {
    id: 'prod-spices-cardamom',
    retailerId: 'store-natures-fresh',
    storeId: 'store-natures-fresh',
    storeName: "Nature's Fresh Grocery",
    storeLocality: 'Indiranagar',
    categoryId: 'cat-grocery',
    categoryName: 'Grocery & Essentials',
    subCategory: 'Spices & Seasoning',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1596040033229-a9821ebd058d?w=200&auto=format&fit=crop&q=80',
    name: 'Whole Green Cardamom (Elaichi 8mm Bold)',
    variantLabel: '8mm Bold Grade',
    unitType: 'weight',
    unitOptions: [25, 50, 100, 250, 500],
    weightUnit: 'g',
    slug: 'green-cardamom-elaichi-bold',
    brand: 'Kerala Spices Co.',
    description: 'Aromatic bold 8mm single-origin Idukki green cardamom pods. Sun-dried and hand graded for intense essential oil and fragrance.',
    price: 240,
    sellingPrice: 195,
    mrp: 290,
    discountPercent: 32,
    inStock: true,
    stockQuantity: 80,
    unit: 'Sold by Weight (50g base)',
    packInfo: 'Origin: Idukki, Kerala | Grade: 8mm Jumbo',
    images: [
      'https://images.unsplash.com/photo-1596040033229-a9821ebd058d?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Grade', value: '8mm Jumbo Green' },
      { label: 'Aroma', value: 'High Cineole Essential Oil' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 220,
    deliveryEtaMin: 12
  },
  {
    id: 'prod-dryfruits-almonds',
    retailerId: 'store-natures-fresh',
    storeId: 'store-natures-fresh',
    storeName: "Nature's Fresh Grocery",
    storeLocality: 'Indiranagar',
    categoryId: 'cat-grocery',
    categoryName: 'Grocery & Essentials',
    subCategory: 'Dry Fruits & Nuts',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1508061252966-ef7fb9677855?w=200&auto=format&fit=crop&q=80',
    name: 'Premium California Non-Pareil Raw Almonds (Badam)',
    variantLabel: 'Premium Nonpareil Grade',
    unitType: 'weight',
    unitOptions: [100, 250, 500, 1000],
    weightUnit: 'g',
    slug: 'california-raw-almonds-badam',
    brand: 'NuttyDelight',
    description: 'Crisp, crunchy, jumbo-sized California Nonpareil grade almonds. 100% natural, unpolished and gluten-free, rich in protein, Vitamin E and dietary fiber.',
    price: 320,
    sellingPrice: 280,
    mrp: 390,
    discountPercent: 28,
    inStock: true,
    stockQuantity: 120,
    unit: 'Sold by Weight (250g base)',
    packInfo: 'Vacuum Packed | 100% Natural Raw',
    images: [
      'https://images.unsplash.com/photo-1508061252966-ef7fb9677855?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Grade', value: 'California Jumbo Nonpareil' },
      { label: 'Nutrition', value: 'High Protein & Vitamin E' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 310,
    deliveryEtaMin: 12
  },
  {
    id: 'prod-dairy-paneer',
    retailerId: 'store-natures-fresh',
    storeId: 'store-natures-fresh',
    storeName: "Nature's Fresh Grocery",
    storeLocality: 'Indiranagar',
    categoryId: 'cat-grocery',
    categoryName: 'Grocery & Essentials',
    subCategory: 'Dairy & Breakfast',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1628088062854-d1870b4553da?w=200&auto=format&fit=crop&q=80',
    name: 'Fresh Malai Paneer (Cream Cottage Cheese)',
    variantLabel: '200g Fresh Block',
    unitType: 'weight',
    unitOptions: [200, 500, 1000],
    weightUnit: 'g',
    slug: 'fresh-malai-paneer-block',
    brand: 'Dairy Pure',
    description: 'Ultra-soft, creamy, melt-in-mouth fresh malai paneer made from pure whole cow milk with zero preservatives or additives.',
    price: 95,
    sellingPrice: 85,
    mrp: 110,
    discountPercent: 22,
    inStock: true,
    stockQuantity: 50,
    unit: 'Sold by Weight (200g base)',
    packInfo: 'Storage: Keep Refrigerated | Shelf Life: 10 Days',
    images: [
      'https://images.unsplash.com/photo-1628088062854-d1870b4553da?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Texture', value: 'Super Soft Malai' },
      { label: 'Protein', value: '18g Protein per 100g' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 195,
    deliveryEtaMin: 12
  },

  // TechHub Electronics Products
  {
    id: 'prod-anker-65w-gan',
    retailerId: 'store-techhub',
    storeId: 'store-techhub',
    storeName: 'TechHub Electronics',
    storeLocality: 'MG Road',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    subCategory: 'Chargers & Adapters',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1583863788434-e58a36330cf0?w=200&auto=format&fit=crop&q=80',
    name: 'Anker 65W GaN III Dual USB-C Fast Wall Charger',
    variantLabel: '65W Dual Port (Black)',
    unitType: 'count',
    unitOptions: [1, 2, 3],
    slug: 'anker-65w-gan-fast-charger',
    brand: 'Anker',
    description: 'Pocket-sized GaN III high-speed 65W PowerIQ 3.0 dual-port charger. Fast charges MacBook Pro, iPhone 16/15, Galaxy S24 and iPad simultaneously.',
    price: 2999,
    sellingPrice: 2499,
    mrp: 3499,
    discountPercent: 28,
    inStock: true,
    stockQuantity: 30,
    unit: '1 Unit',
    packInfo: 'Includes Foldable US/EU Pins Adapter',
    images: [
      'https://images.unsplash.com/photo-1583863788434-e58a36330cf0?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Output', value: '65W Max Power Delivery' },
      { label: 'Technology', value: 'GaN III Fast & Cool Charging' },
      { label: 'Ports', value: '2x USB-C + 1x USB-A' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 94,
    deliveryEtaMin: 30
  },
  {
    id: 'prod-braided-usbc-cable',
    retailerId: 'store-techhub',
    storeId: 'store-techhub',
    storeName: 'TechHub Electronics',
    storeLocality: 'MG Road',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    subCategory: 'Cables & Hubs',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=200&auto=format&fit=crop&q=80',
    name: 'Braided 100W USB-C to USB-C Fast Charging Cable (2m)',
    variantLabel: '2m Braided Nylon',
    unitType: 'count',
    unitOptions: [1, 2, 3, 5],
    slug: 'braided-100w-usbc-cable-2m',
    brand: 'Baseus',
    description: 'Heavy-duty double-braided nylon 100W 5A E-Marker fast charging cable with reinforced aluminum alloy connector housings rated for 30,000+ bends.',
    price: 699,
    sellingPrice: 499,
    mrp: 899,
    discountPercent: 44,
    inStock: true,
    stockQuantity: 85,
    unit: '1 Cable',
    packInfo: 'Length: 2 Meters | 100W PD Supported',
    images: [
      'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Power', value: '100W (20V/5A)' },
      { label: 'Data Speed', value: '480 Mbps' },
      { label: 'Durability', value: '30,000 Bends Tested' }
    ],
    isPopular: true,
    rating: 4.7,
    reviewCount: 165,
    deliveryEtaMin: 30
  },
  {
    id: 'prod-usbc-6in1-hub',
    retailerId: 'store-techhub',
    storeId: 'store-techhub',
    storeName: 'TechHub Electronics',
    storeLocality: 'MG Road',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    subCategory: 'Cables & Hubs',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1526738549149-8e07eca6c147?w=200&auto=format&fit=crop&q=80',
    name: 'Aluminum 6-in-1 USB-C Hub with 4K HDMI & 100W PD',
    variantLabel: 'Space Grey Aluminum',
    unitType: 'count',
    unitOptions: [1, 2],
    slug: 'aluminum-6in1-usbc-hub-4k-hdmi',
    brand: 'UGREEN',
    description: 'Premium space-grey aerospace aluminum hub with 4K@60Hz HDMI, 100W Power Delivery pass-through, SD/TF card slots and 2x USB 3.0 5Gbps ports.',
    price: 2499,
    sellingPrice: 1999,
    mrp: 2999,
    discountPercent: 33,
    inStock: true,
    stockQuantity: 20,
    unit: '1 Hub',
    packInfo: 'Plug & Play | No Driver Required',
    images: [
      'https://images.unsplash.com/photo-1526738549149-8e07eca6c147?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Video', value: '4K @ 60Hz HDMI HDR' },
      { label: 'Pass-through', value: '100W USB-C PD' },
      { label: 'Speed', value: '5Gbps SuperSpeed USB 3.0' }
    ],
    isPopular: false,
    rating: 4.8,
    reviewCount: 52,
    deliveryEtaMin: 30
  },

  // ABC Electrical Store Products
  {
    id: 'prod-gm-power-strip',
    retailerId: 'store-abc-electrical',
    storeId: 'store-abc-electrical',
    storeName: 'ABC Electrical Store',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Sockets & Surge Protection',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=200&auto=format&fit=crop&q=80',
    name: 'GM Modular 4-Way Power Strip with Surge Protector',
    variantLabel: '4-Socket + Master Switch',
    unitType: 'count',
    unitOptions: [1, 2, 3],
    slug: 'gm-modular-4way-power-strip',
    brand: 'GM Modular',
    description: 'Ergonomic 4-outlet surge protector spike guard with master switch, safety shutter, and 2-meter heavy gauge power cord with thermal trip protection.',
    price: 590,
    sellingPrice: 479,
    mrp: 650,
    discountPercent: 26,
    inStock: true,
    stockQuantity: 45,
    unit: '1 Unit',
    packInfo: 'Cord Length: 2m | 10A 2400W Surge Rating',
    images: [
      'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Outlets', value: '4 International Sockets' },
      { label: 'Protection', value: 'MOV Surge & Spike Suppressor' },
      { label: 'Safety', value: 'Child Safety Shutters' }
    ],
    isPopular: true,
    rating: 4.7,
    reviewCount: 78,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-wipro-smart-rgb',
    retailerId: 'store-abc-electrical',
    storeId: 'store-abc-electrical',
    storeName: 'ABC Electrical Store',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'LED Bulbs & Lighting',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1550985543-f47f38aeee65?w=200&auto=format&fit=crop&q=80',
    name: 'Wipro Next 9W Smart RGB+W B22 LED Bulb',
    variantLabel: '9W Smart 16M Colors',
    unitType: 'count',
    unitOptions: [1, 2, 4],
    slug: 'wipro-next-9w-smart-rgb-bulb',
    brand: 'Wipro',
    description: 'Wi-Fi enabled smart LED bulb with 16 million colors, tuneable white (2700K - 6500K), dimming control and Amazon Alexa & Google Assistant voice compatibility.',
    price: 799,
    sellingPrice: 599,
    mrp: 990,
    discountPercent: 39,
    inStock: true,
    stockQuantity: 35,
    unit: '1 Bulb',
    packInfo: 'Base: B22 | Works with Wi-Fi (No Hub Needed)',
    images: [
      'https://images.unsplash.com/photo-1550985543-f47f38aeee65?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Colors', value: '16 Million Colors + Tunable White' },
      { label: 'Control', value: 'App & Alexa/Google Voice' },
      { label: 'Scheduling', value: 'Timer & Music Sync' }
    ],
    isPopular: true,
    rating: 4.8,
    reviewCount: 142,
    deliveryEtaMin: 20
  },
  {
    id: 'prod-taparia-tester',
    retailerId: 'store-abc-electrical',
    storeId: 'store-abc-electrical',
    storeName: 'ABC Electrical Store',
    storeLocality: 'Indiranagar',
    categoryId: 'cat-electrical',
    categoryName: 'Electrical',
    subCategory: 'Electrical Tools & Testers',
    subCategoryImageUrl: 'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=200&auto=format&fit=crop&q=80',
    name: 'Taparia 814 Line Tester & Electric Voltage Pen',
    variantLabel: 'Standard 500V Tester',
    unitType: 'count',
    unitOptions: [1, 2, 5],
    slug: 'taparia-814-line-tester',
    brand: 'Taparia',
    description: 'Heavy duty high-visibility neon indicator line tester pen with insulated cellulose acetate body and pocket clip. Tested up to 500V AC.',
    price: 65,
    sellingPrice: 50,
    mrp: 75,
    discountPercent: 33,
    inStock: true,
    stockQuantity: 100,
    unit: '1 Pc',
    packInfo: 'Voltage: 100V - 500V AC | IS:5579 Certified',
    images: [
      'https://images.unsplash.com/photo-1581783342308-f792dbdd27c5?w=800&auto=format&fit=crop&q=80'
    ],
    features: [
      { label: 'Voltage Range', value: '100V to 500V AC' },
      { label: 'Material', value: 'High Grade CA Plastic Handle' }
    ],
    isPopular: true,
    rating: 4.9,
    reviewCount: 88,
    deliveryEtaMin: 20
  }
];

export const INITIAL_COUPONS: Coupon[] = [
  {
    code: 'GETORA100',
    description: 'Flat ₹100 off on your first local store order above ₹299',
    discount: 100,
    discountType: 'fixed',
    discountValue: 100,
    minOrderValue: 299,
    maxDiscount: 100,
    validUntil: '2026-12-31'
  },
  {
    code: 'LOCALFAST',
    description: '20% off on all neighborhood electronics & electrical items',
    discount: 20,
    discountType: 'percentage',
    discountValue: 20,
    minOrderValue: 199,
    maxDiscount: 150,
    validUntil: '2026-12-31'
  },
  {
    code: 'FREEDEL',
    description: 'Free Delivery on any local order above ₹149',
    discount: 40,
    discountType: 'fixed',
    discountValue: 40,
    minOrderValue: 149,
    maxDiscount: 40,
    validUntil: '2026-12-31'
  }
];

export const INITIAL_ADDRESSES: Address[] = [
  {
    id: 'addr-home',
    tag: 'Home',
    addressType: 'Home',
    addressLine1: 'A-42, Tech Park Enclave',
    flatNo: 'A-42, Tech Park Enclave',
    streetArea: '100 Feet Road, HAL 2nd Stage, Indiranagar',
    landmark: 'Opp. BDA Complex',
    city: 'Bengaluru',
    state: 'Karnataka',
    pincode: '560038',
    isDefault: true
  },
  {
    id: 'addr-work',
    tag: 'Work',
    addressType: 'Work',
    addressLine1: 'Tower B, 4th Floor, Nexus Coworking',
    flatNo: 'Tower B, 4th Floor, Nexus Coworking',
    streetArea: '80 Feet Road, Koramangala 4th Block',
    landmark: 'Near Sony World Junction',
    city: 'Bengaluru',
    state: 'Karnataka',
    pincode: '560034',
    isDefault: false
  }
];

export const INITIAL_ORDERS: Order[] = [
  {
    id: 'ord-883921',
    orderNumber: 'GET-883921',
    customerId: 'cust-demo-01',
    retailerId: 'store-voltix',
    addressId: 'addr-home',
    storeId: 'store-voltix',
    storeName: 'Voltix Electricals',
    storeAddress: '14, 80 Feet Road, Koramangala',
    items: [
      {
        product: INITIAL_PRODUCTS[1], // Philips Bulb
        quantity: 2
      },
      {
        product: INITIAL_PRODUCTS[4], // Legrand Plate
        quantity: 1
      }
    ],
    orderStatus: 'out_for_delivery',
    status: 'out_for_delivery',
    subtotal: 718,
    deliveryFee: 40,
    platformFee: 5,
    discount: 100,
    tax: 0,
    totalAmount: 663,
    grandTotal: 663,
    couponCode: 'GETORA100',
    paymentMethod: 'UPI',
    paymentStatus: 'paid',
    deliveryAddress: INITIAL_ADDRESSES[0],
    deliveryInstructions: 'Ring bell and leave at door',
    deliveryPartner: {
      id: 'd1111111-1111-1111-1111-111111111111',
      name: 'Rahul Sharma',
      phone: '+91 98765 43210',
      vehicle: 'Hero Electric Optima (KA-05-EV-4421)',
      rating: 4.9,
      totalDeliveries: 1420,
      avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=200&auto=format&fit=crop&q=80'
    },
    estimatedDeliveryTime: '12 mins (Around 10:45 PM)',
    placedAt: new Date(Date.now() - 1000 * 60 * 12).toISOString(),
    createdAt: new Date(Date.now() - 1000 * 60 * 12).toISOString(),
    updatedAt: new Date(Date.now() - 1000 * 60 * 12).toISOString(),
    driverLocation: {
      lat: 12.9725,
      lng: 77.6410,
      progressPercent: 70
    }
  },
  {
    id: 'ord-774102',
    orderNumber: 'GET-774102',
    customerId: 'cust-demo-01',
    retailerId: 'store-natures-fresh',
    addressId: 'addr-home',
    storeId: 'store-natures-fresh',
    storeName: "Nature's Fresh Grocery",
    storeAddress: '88, 12th Main, Indiranagar',
    items: [
      {
        product: INITIAL_PRODUCTS[6], // Organic Broccoli
        quantity: 2
      },
      {
        product: INITIAL_PRODUCTS[7], // Alphonso Mangoes
        quantity: 1
      }
    ],
    orderStatus: 'delivered',
    status: 'delivered',
    subtotal: 877,
    deliveryFee: 0,
    platformFee: 5,
    discount: 50,
    tax: 0,
    totalAmount: 832,
    grandTotal: 832,
    paymentMethod: 'UPI',
    paymentStatus: 'paid',
    deliveryAddress: INITIAL_ADDRESSES[0],
    deliveryPartner: {
      id: 'd2222222-2222-2222-2222-222222222222',
      name: 'Vikram Patel',
      phone: '+91 98111 22334',
      vehicle: 'Ather 450X (KA-01-EQ-9090)',
      rating: 4.8,
      totalDeliveries: 980,
      avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&auto=format&fit=crop&q=80'
    },
    estimatedDeliveryTime: 'Delivered',
    placedAt: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(),
    createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(),
    updatedAt: new Date(Date.now() - 1000 * 60 * 60 * 23.5).toISOString(),
    deliveredAt: new Date(Date.now() - 1000 * 60 * 60 * 23.5).toISOString()
  }
];
